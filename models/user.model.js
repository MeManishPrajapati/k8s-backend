...
userSchema.post('validate', async function (error, doc, next) {
    if (error)
        console.log("Error while validating data:", JSON.stringify(this));
    else
        console.log("Validated data:", JSON.stringify(this));
    next()
})
...
userSchema.pre('findOneAndUpdate', function (next) {
    console.log(`Got request to find: ${JSON.stringify(this.getFilter())} and update: ${JSON.stringify(this.getUpdate())}`);
    next()
});
